...
- **Next:** Add explicit composite/OR criterion representation before treating
  ...
  review churn. A standalone composite truth-table helper is acceptable before
  scorer wiring, but `match_extracted` and the case rollup must stay unchanged
  until the extractor/fixer can emit parent/subcheck groups reliably.
