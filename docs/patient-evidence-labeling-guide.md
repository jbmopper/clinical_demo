...
| **Free-text criterion** | Often **human_review_required**; use **insufficient_evidence** or expected indeterminate unless retrieved note rows or composite subcheck rows provide citeable evidence. If the criterion is actually correlatable to structured condition, medication, measurement, temporal, or trial-exposure evidence, note that in `rationale` so the row feeds the mapping/retrieval backlog. |
