...
- **Composite OR/AND criteria** now have a native extractor/fixer parent/subcheck contract, calibration-time scaffolding, and deterministic matcher semantics for flat `any_of` / `all_of` groups. Nested groups and rich event extraction remain out of scope.
